...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const deleteChatHistory = (index) => {
    ...
    if (index >= 0 && index < chatHistory.length) {
      ...
      // 更新 localStorage 中的对话历史
      localStorage.setItem('chatHistory', JSON.stringify(chatHistory));
      
      // 刷新页面中的对话列表
      conversationListElement.innerHTML = ''; // 清空当前列表
      loadConversationList(); // 重新加载对话列表
      
      // 如果删除的是当前正在显示的对话，清空聊天记录
      const currentChatLog = JSON.parse(localStorage.getItem('chatLog')) || [];
      if (currentChatLog.length > 0 && index === chatHistory.length) {
        localStorage.removeItem('chatLog');
        chatLogElement.innerHTML = '';
      }
    } else {
